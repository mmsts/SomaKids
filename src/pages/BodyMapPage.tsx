...
export function BodyMapPage({ onComplete }: Props) {
  ...
  const handleSubmitExpression = async () => {
    ...
    try {
      ...
      // API 失败时用本地规则引擎生成 fallback 追问，不中断流程
      const localResult = generateLocalHypothesis(conv.part, trimmed, []);
      setConv((prev) => ({
        ...prev,
        hypothesis: localResult.hypothesis,
        ...
        history: [
          ...
          {
            role: 'companion',
            content: `${localResult.hypothesis} ${localResult.question}`,
            type: 'followup',
          },
        ],
      }));
  ...
  const handleYes = async () => {
    ...
    try {
      ...
      // API 失败时用本地规则引擎生成 fallback 追问
      const localResult = generateLocalHypothesis(
        conv.part!,
        conv.initialExpression,
        conv.rejected
      );
      setConv((prev) => ({
        ...prev,
...
// ───── Local Fallback — API 失败时用规则引擎生成追问 ───────────

function generateLocalHypothesis(
  part: BodyPart,
  expression: string,
  rejected: string[]
): { hypothesis: string; question: string; confidence: number } {
  const result = analyzeInput(part, expression);
  const partLabel = BODY_LABEL[part];

  // 找到未被拒绝的身体信号
  const availableSignals = result.bodySignals.filter(
    (s) => !rejected.some((r) => r.includes(s.symptom))
  );

  const topSignal = availableSignals[0];

  if (topSignal) {
    return {
      hypothesis: `我觉得你的${partLabel}可能是${topSignal.symptom}的感觉…`,
      question: '是这样的感觉吗？',
      confidence: Math.round(topSignal.confidence * 100),
    };
  }
  ...
  if (result.riskLevel === 'low') {
    return {
      hypothesis: `你的${partLabel}在传递一些感受，让我再仔细听一听…`,
      question: '可以再说得具体一点点吗？',
      confidence: 30,
    };
  }

  return {
    hypothesis: `你的${partLabel}似乎有些不太舒服，我想更懂你一点…`,
    question: '是痛痛的感觉，还是胀胀的呀？',
    confidence: 40,
  };
}
